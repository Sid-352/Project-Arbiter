...
fn main() {
    ...
    // Load Signet vault
        ...
        let (atlas_exec_tx, mut atlas_exec_rx) = tokio::sync::mpsc::channel::<vassal_core::ordinance::ExecData>(16);
        ...
        vassal_bridge::executor::spawn_executor(
            exec_cmd_rx,
            1920, 1080, // Hardcoded display resolution for Phase 2
            filter.clone(),
        );
        ...
        // Just an example to prove compilation and logic — assumes Downloads exist.
            if let Some(mut downloads) = dirs::download_dir() {
        ...
        if let Some(archive_dir) = dirs::document_dir().map(|d| d.join("Vassal_Archives")) {
            ...
            let fs_nodes = vec![
                ...
                OrdNode {
                    id: "2".into(), label: "Move File".into(), kind: NodeKind::Action,
